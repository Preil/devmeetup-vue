<template>
  <v-dialog persistent v-model="registerDialog">
    <v-btn class="primary" accent slot="activator">{{ userIsRegistered ? 'Unregister' : 'Register'}}</v-btn>
    <v-card>
      <v-container>
        <v-layout row wrap>
          <v-flex xs12>
        ...
        <v-layout row wrap>
          <v-flex xs12>
            <v-btn flat class="red--text darken-1" @click="registerDialog=false">Close</v-btn>
